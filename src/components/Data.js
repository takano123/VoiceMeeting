    ...
    switch(props.typeData){
      case '1':
        ...
        props.Department.map((dep)=>{
          ...
          // Scoresの各要素をMaxScoresの対応する要素で割り、100倍する
          for (const key in result) {
            if (result.hasOwnProperty(key) && MaxScores.hasOwnProperty(key)) {
              scoresObj[key] = Math.round((result[key] / MaxScores[key]) * 1000) / 10;
            }
          }
